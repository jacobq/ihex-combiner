                ...
                switch (intelHexRecord.RecordType)
                {
                    case IntelHexRecordType.Data:
                        if (shouldSetStartAddressAtNextRecord)
                        {
                            ...
                            Console.WriteLine($"Setting startAddress to {string.Format("0x{0,8:X8}", startAddress)}"); // DEBUG
                        ...
                        if (i != offset)
                            Console.WriteLine($"offset = {offset}, i = {i}; {hexStreamReader.State}"); // DEBUG
                        while (i < offset)
                            bytes[i++] = (byte)0xFF;
                    ...
                    case IntelHexRecordType.EndOfFile:
                        //Console.WriteLine("EOF " + hexStreamReader.State); // DEBUG
                        break;
                    case IntelHexRecordType.ExtendedSegmentAddress:
                        //Console.WriteLine("ExtendedSegmentAddress " + hexStreamReader.State); // DEBUG
                        break;
                    case IntelHexRecordType.StartSegmentAddress:
                        //Console.WriteLine("StartSegmentAddress " + hexStreamReader.State); // DEBUG
                        break;
                    case IntelHexRecordType.ExtendedLinearAddress:
                        //Console.WriteLine("ExtendedLinearAddress " + hexStreamReader.State); // DEBUG
                        break;
                    case IntelHexRecordType.StartLinearAddress:
                        //Console.WriteLine("StartLinearAddress " + hexStreamReader.State); // DEBUG
                        break;
                    default:
